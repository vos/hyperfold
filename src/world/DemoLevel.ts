...
function fillBox(grid: number[][], r1: number, c1: number, r2: number, c2: number, tile: TileType) {
  for (let r = r1; r <= r2; r++) {
    for (let c = c1; c <= c2; c++) {
...
function addEnclosure(grid: number[][], exits: { left: boolean; right: boolean; up: boolean; down: boolean }) {
  // Top wall
  for (let c = 0; c < COLS; c++) {
...
export function buildDemoLevel(): LevelMap {
  ...
  // ==========================================
  {
    ...
    fillBox(tiles, 15, 2, 15, 5, TileType.ONE_WAY);
    fillBox(tiles, 11, 3, 11, 6, TileType.ONE_WAY);
    fillBox(tiles, 11, 13, 11, 16, TileType.ONE_WAY);
    fillBox(tiles, 7, 4, 7, 7, TileType.ONE_WAY);
    fillBox(tiles, 7, 12, 7, 15, TileType.ONE_WAY);
  ...
  // ==========================================
  {
    ...
    // Left ledge
    fillBox(tiles, 18, 0, 18, 5, TileType.SOLID);
    // Right ledge
    fillBox(tiles, 18, 14, 18, 19, TileType.SOLID);

    // Stepping stones to and from crumble platforms
    tiles[17][5] = TileType.SOLID;
    tiles[17][14] = TileType.SOLID;
  ...
  // ==========================================
  {
    ...
    fillBox(tiles, 17, 13, 17, 17, TileType.SOLID);
    fillBox(tiles, 16, 14, 16, 16, TileType.SOLID);
    tiles[15][15] = TileType.GOAL;

    // Platform leadup
    fillBox(tiles, 15, 5, 15, 8, TileType.ONE_WAY);
    fillBox(tiles, 13, 9, 13, 11, TileType.ONE_WAY);
  ...
  // ==========================================
  {
    ...
    fillBox(tiles, 15, 13, 15, 15, TileType.SOLID);

    // One-way landing platform over the entrance chute (allows jumping through from (2,1)!)
    fillBox(tiles, 16, 7, 16, 12, TileType.ONE_WAY);

    // High floating clouds/platforms
    fillBox(tiles, 13, 3, 13, 7, TileType.SOLID);
    fillBox(tiles, 13, 12, 13, 16, TileType.SOLID);
    fillBox(tiles, 9, 8, 9, 11, TileType.SOLID);
