...
const DevManager_1 = require("./DevManager");
class PhysicsEngine {
    audio;
    particles;
    devManager = DevManager_1.DevManager.getInstance();
            ...
            if (this.isExitOpen(room, 'down', levelMap)) {
                ...
                if (gate && !(levelMap || this.levelMap)?.hasKey(gate.id)) {
                    ...
                    if (this.devManager.enabled && this.devManager.godMode) {
                        player.y = ScreenData_1.FACE_SIZE - player.height - 10;
                        player.vy = -600;
                        this.audio.playBounce();
                        return null;
                    }
    ...
    checkLaserBarriers(player, room, onPlayerDeath, dt = 0.016) {
